...
dR2dt = comparator.AddCompositor('R2', 1000);
...
comparator.AddPart(Part('R1', dR1dt,Rate('-A1*w1*sin(w1*T1 + pi)')));
...
detection_threshold = 250*ones(length(T));
upper_threshold = 1000*ones(length(T));
lower_threshold = 100*ones(length(T));
...
figure()
hold on
plot(T,Y(:,comparator.CompositorIndex('R1')),'m',... %T,Y(:,comparator.CompositorIndex('R2')),'c',...
    T,Y(:,comparator.CompositorIndex('GFP')),'g',T,Y(:,comparator.CompositorIndex('BFP')),'b',...
    ... %T,Y(:,comparator.CompositorIndex('RFP')),'r',...
    'LineWidth',2)
xlabel('Minutes'); ylabel('Molecules/Cell'); title('Magnitude Comparator: Oscillating Input')
legend('R1','GFP','BFP')
%plot(T,detection_threshold,'k--')